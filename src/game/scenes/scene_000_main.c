...
void scene_000_ondraw(Scene *s, SDL_Renderer *r)
{
    ...
    Collider *btn_left  = (Collider *)hash_map_get(s->colliders, SCENE_000_BTN_LEFT);
    Collider *btn_mid   = (Collider *)hash_map_get(s->colliders, SCENE_000_BTN_MID);
    Collider *btn_right = (Collider *)hash_map_get(s->colliders, SCENE_000_BTN_RIGHT);
