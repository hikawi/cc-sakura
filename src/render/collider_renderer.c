 ...
 */
void render_filled_half_circle(SDL_FPoint center, SDL_FColor fcolor,
                               double radius, double theta)
{
    ...
    SDL_RenderGeometry(renderer, NULL, vertices, (int)segments + 2, triangles,
                       (int)segments * 3);
 ...
 */
void render_rotated_rectangle(SDL_Color color, Vector2 points[4],
                              Vector2 origin, double angle)
{
    SDL_Renderer *renderer = get_current_renderer();
...
void render_circle_collider(Collider *collider)
{
    ...
    render_filled_half_circle((SDL_FPoint){(float)circ.x, (float)circ.y},
                              fcolor, circ.r, 0);
    render_filled_half_circle((SDL_FPoint){(float)circ.x, (float)circ.y},
                              fcolor, circ.r, -M_PI);
}
