...
bool init_window_and_renderer(void)
{
    return SDL_CreateWindowAndRenderer(
        ...
        APPLICATION_ORIGINAL_HEIGHT, SDL_WINDOW_RESIZABLE, &window, &renderer);
}
...
void render_aligned_texture(RenderingOptions options)
{
    ...
    {
        SDL_FlipMode flag =
            options.flipped_horizontally ? SDL_FLIP_HORIZONTAL : 0;
    ...
    else if (options.is_tiled)
    {
        if (options.scale.x != options.scale.y)
        {
            SDL_LogWarn(SDL_LOG_CATEGORY_RENDER,
                        "Tiling texture's scale is not equal. Using scale=%.2f",
                        options.scale.x);
        }

        SDL_RenderTextureTiled(renderer, options.texture, options.srcrect,
                               (float)options.scale.x, &dstrect);
    }
    else
    {
...
void render(AppState *state)
{
    ...
    if (APPLICATION_SHOW_FPS)
    {
        ...
        draw_font(FONT_FACE_DAYDREAM, buf, w / 2.0, 10, color,
    ...
    // Render Sakura's position.
    {
        ...
        draw_font(FONT_FACE_RAINY_HEARTS, buf, w / 2.0, 50, color,
                  RENDER_ORIGIN_TOP_CENTER);
    }
