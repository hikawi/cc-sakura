 ...
 *
 * For each sprite (each line), this goes (starting x, starting y, width,
 * height) for the sprite itself only, cut out borders and paddings.
 *
 * For the values that have additions, it means "starting frame box" + "sprite
 * offset", since some sprites might be in the middle, and don't cover the
 * entire 64x64 box.
 */
static const int SAKURA_IDLE_ANIMATION_BOUNDARY[][4] = {
    {0, 0 + 0, 24, 64},   {24, 0 + 1, 24, 64}, {48, 0 + 0, 24, 64},
    {72, 0 + 0, 24, 64},  {96, 0 + 0, 24, 64}, {120, 0 + 0, 24, 64},
    {144, 0 + 0, 24, 64},
};
...
bool init_sakura_sprites(void)
{
    ...
    if (sakura_txt != NULL)
    {
        SDL_LogWarn(SDL_LOG_CATEGORY_SYSTEM,
                    "Sakura spritesheet already loaded. Skipping.");
        return true; // Don't do anything if already loaded
    }

    SpriteTexture sprtxt =
        load_texture_from_image(renderer, SAKURA_SPRITESHEET_PATH);
    if (sprtxt.texture == NULL)
    {
        SDL_LogError(SDL_LOG_CATEGORY_SYSTEM, "Failed to load file %s",
                     SAKURA_SPRITESHEET_PATH);
        return false;
    }
...
bool init_sakura(void)
{
    ...
    {
        SDL_LogWarn(SDL_LOG_CATEGORY_SYSTEM,
                    "Sakura's sprites are not loaded yet. Loading them now.");
    ...
    {
        SDL_LogWarn(SDL_LOG_CATEGORY_SYSTEM,
                    "Sakura already initialized. Skipping.");
        return true;
    }
    ...
    sakura->idle->textures =
        malloc(sizeof(SpriteTexture) * SAKURA_IDLE_ANIMATION_LENGTH);
...
void fixed_update_sakura(AppState *app)
{
        ...
        if (info.is_colliding)
        {
            ...
            if (SDL_fabs(info.normal.x) > SDL_fabs(info.normal.y))
                if (!deepest_x_info.is_colliding ||
                    info.depth > deepest_x_info.depth)
                    deepest_x_info = info;
        }
    }
    ...
    if (deepest_x_info.is_colliding)
    {
        skr->pos = add_vector2(skr->pos, scale_vector2(deepest_x_info.normal,
                                                       deepest_x_info.depth));
        skr->velo.x = 0;
        move_sakura_collider();
    }
        ...
        if (info.is_colliding)
        {
            if (SDL_fabs(info.normal.y) > SDL_fabs(info.normal.x))
                if (!deepest_y_info.is_colliding ||
                    info.depth > deepest_y_info.depth)
                    deepest_y_info = info;
        }
    }
    ...
    if (deepest_y_info.is_colliding)
    {
        skr->pos = add_vector2(skr->pos, scale_vector2(deepest_y_info.normal,
                                                       deepest_y_info.depth));
        skr->velo.y = 0;
        move_sakura_collider();
        SDL_Log("Resolving collision of depth %.2f at (%.2f, %.2f)",
                deepest_y_info.depth, deepest_y_info.normal.x,
                deepest_y_info.normal.y);
    }
    destroy_collider_list(list);
    ...
    // Find the feet collision. Since she's a capsule, she can be rotated
    // arbitrarily, therefore, we get her feet by averaging her x axis, and take
    // the larger of her y coords + alpha.
    AABBCollider feet;
    feet.x = (skr->collider->capsule.p1.x + skr->collider->capsule.p2.x) / 2;
    feet.y = SDL_max(skr->collider->capsule.p1.y, skr->collider->capsule.p2.y) +
             APPLICATION_SCALE;
    ...
    for (int i = 0; i < list->length; i++)
    {
        Collision info;
        if (list->list[i] == skr->collider ||
            list->list[i]->collision_type != COLLISION_SOLID)
            continue;
...
void destroy_sakura(void)
{
    ...
    {
        SDL_LogWarn(SDL_LOG_CATEGORY_SYSTEM,
                    "Destroyed Sakura but her textures are still loaded. "
                    "Destroying the textures also.");
