...
engine::engine(engine_deps &&deps) : m_deps(std::move(deps))
{
    ...
    // TODO: Remove
    const scene_handle empty_handle = m_scene_mgr.push_back(std::make_unique<scenes::dbg_empty>(sdl::fcolor(1, 0, 1, 1)));
    m_scene_mgr.start_transition(std::make_unique<scenes::dbg_physics>(), empty_handle,
